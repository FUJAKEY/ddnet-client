...
void SFloatConfigVariable::CommandCallback(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		char aBuf[256];
		pThis->Serialize(aBuf, sizeof(aBuf)); // This serializes the value into aBuf.
		pThis->m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, pThis->m_pScriptName, aBuf); // Pass aBuf to Print.
	}
}
...
void SFloatConfigVariable::SetValue(float Value)
{
	...
	if(Value < m_Min)
		...
		Value = m_Max;
	*m_pVariable = Value;
	str_format(aBuf, sizeof(aBuf), "Value changed to %.5f", *m_pVariable); // Format the string into aBuf.
	m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, m_pScriptName, aBuf); // Pass aBuf to Print.
}
