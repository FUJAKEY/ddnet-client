...
{
	if(g_Config.m_ClAvoidFreeze && m_IsLocal && !m_FreezeTime && !m_Core.m_DeepFrozen && !m_Core.m_LiveFrozen && GameWorld()->GameTick() > m_LastAvoidFreezeHookTick + Client()->GameTickSpeed() / 2)
	{
		...
		for(int i = 0; i < 5; ++i) // Predict up to 5 ticks ahead
		{
			...
			if (Collision()->GetSwitchLayer()) // Check if switch layer exists
			{
				...
				if(pSwitchTile && (pSwitchTile->m_Type == TILE_FREEZE || pSwitchTile->m_Type == TILE_DFREEZE || pSwitchTile->m_Type == TILE_LFREEZE))
				{
					...
					if (pSwitchTile->m_Type == TILE_FREEZE || pSwitchTile->m_Type == TILE_DFREEZE || pSwitchTile->m_Type == TILE_LFREEZE)
						SwitchTile = pSwitchTile->m_Type; // Store the type of freeze tile
				}
			}
			...
			if(MainTile == TILE_FREEZE || MainTile == TILE_DFREEZE || MainTile == TILE_LFREEZE ||
			   ...
			   SwitchTile == TILE_FREEZE || SwitchTile == TILE_DFREEZE || SwitchTile == TILE_LFREEZE)
			{
				m_Input.m_Hook = 1;
			...
			if (OriginalVel.y < 0 && TempCore.m_Vel.y >= 0 && TempCore.m_Grounded)
			{
				break;
			}
		}
